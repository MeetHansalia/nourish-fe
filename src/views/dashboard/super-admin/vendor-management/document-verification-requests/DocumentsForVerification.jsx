...
const DocumentsForVerification = props => {
  ...
  const getSubmittedDocuments = async () => {
    setIsLoading(true)
    await axiosApiCall
      .get(API_ROUTER.SUPER_ADMIN_VENDOR.DOC_APPROVE_REJECT(id))
      .then(response => {
        setIsLoading(false)
    ...
    actionConfirmWithLoaderAlert(
      {
         ...
         */
        deleteUrl: API_ROUTER.SUPER_ADMIN_VENDOR.VENDOR_DOC_APPROVE_REJECT(id),
  ...
  const handleReject = async () => {
    ...
    await axiosApiCall
      .patch(API_ROUTER.SUPER_ADMIN_VENDOR.VENDOR_DOC_APPROVE_REJECT(id), requestBody)
      .then(response => {
