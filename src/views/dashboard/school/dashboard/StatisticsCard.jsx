  ...
  <CardContent className='flex flex-col gap-1'>
    <div className='flex items-center gap-4'>
      ...
      <div className='card-text-top'>
        ...
        <Typography variant='body2' color='text.disabled'>Last Month</Typography>
      </div>
    </div>

    <div className='number-text-block flex flex-col gap-1'>
      <div className='number-text-block-inner flex items-center gap-2'>
        {/* Show two numbers with labels for Food Chart, otherwise show one */}
        {title === "Food Chart" ? (
          <div className="flex justify-between w-full">
            <div className="flex flex-col items-center">
            <Typography variant='body2' color='text.disabled'>Approve</Typography>
              <Typography variant='h4' color='text.primary'>
                {isLoading ? <CircularProgress size={20} /> : numberFormat(value?.leftNumber)}
              </Typography>
            </div>
            <div className="flex flex-col items-center">
            <Typography variant='body2' color='text.disabled'>Reject</Typography>
              <Typography variant='h4' color='text.primary'>
                {isLoading ? <CircularProgress size={20} /> : numberFormat(value?.rightNumber)}
              </Typography>
            </div>
          </div>
        ) : (
