...
const DocumentVerificationTable = props => {
  ...
  const getSubmittedDocuments = async () => {
    ...
    await axiosApiCall
      .get(API_ROUTER.ADMIN.DOC_APPROVE_REJECT(id), {
        // params: { userId: id }
      })
      .then(response => {
    ...
    actionConfirmWithLoaderAlert(
      {
         ...
         */
        deleteUrl: API_ROUTER.ADMIN.VENDOR_DOC_APPROVE_REJECT(id),
  ...
  const handleReject = async () => {
    ...
    await axiosApiCall
      .patch(API_ROUTER.ADMIN.VENDOR_DOC_APPROVE_REJECT(id), requestBody)
      .then(response => {
  ...
  return (
    <Card p={3}>
      <CardHeader
        ...
        action={
          <div className='flex gap-2'>
            <Button variant='contained' color='primary' onClick={handleApprove} disabled={isLoadingDocuments}>
      ...
      />
      <CardContent>
        <Box display='flex' justifyContent='flex-end' gap={1} mb={2}></Box>
