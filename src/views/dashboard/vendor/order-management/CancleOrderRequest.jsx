...
const OrdersTableComponent = ({ mode, dictionary }) => {
  ...
  const handleAction = async (action, schoolId, orders) => {
    try {
      ...
      // setData(prevData =>
      //   prevData.map(item =>
      //     item.orders[0].id === row.original.orders[0].id
          ...
          <Box display='flex' flexDirection='column' gap={1}>
            ...
            <Box display='flex' gap={1}>
              <Button variant='contained' color='primary'>
                {row?.original?.orders[0]?.cancelorderRequestStatus == 'initiate'
                  ? 'Pending'
                  : row?.original?.orders[0]?.cancelorderRequestStatus}
              </Button>
            </Box>
          </Box>
        )
  ...
  const getDataForInquiryTable = async () => {
    ...
    try {
      const response = await axiosApiCall.get(API_ROUTER.VENDOR.GET_CANCELLED_ORDERS, {
        params: {
          page,
          limit: itemsPerPage,
