...
const OrdersTableComponent = ({ mode, dictionary }) => {
  ...
  const handleAction = async (action, schoolId, orders) => {
    try {
      ...
      const parsedDate = moment(orders[0].orderDate).format('YYYY-MM-DD')
        ...
        cell: ({ row }) => (
          <Box display='flex' flexDirection='column' gap={1}>
            ...
            <Box display='flex' gap={1}>
              <Button variant='contained' color='primary' disabled={row?.original?.status === 'Accepted'}>
                Active
              </Button>
              <Button
                variant='outlined'
  ...
  const getDataForInquiryTable = async () => {
    ...
    try {
      const response = await axiosApiCall.get(API_ROUTER.VENDOR.GET_EVENT_ORDERS, {
        params: {
          page,
          limit: itemsPerPage,
