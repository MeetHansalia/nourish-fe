...
const SuggestionsOnMenu = props => {
  ...
  const getAllSuspendedUsers = async () => {
    ...
    try {
      const response = await axiosApiCall.get(API_ROUTER.DISTRICT.GET_SUGGESTION_VENDOR_LIST, {
        params: {
          page,
          limit: itemsPerPage,
  ...
  const columns = useMemo(
    () => [
      ...
      columnHelper.accessor(row => `${row.vendor.first_name} ${row.vendor.last_name}`, {
        header: `${dictionary?.datatable?.column?.vendor_name}`,
        cell: info => info.getValue() || 'N/A'
      }),
      columnHelper.accessor('suggestion', {
        header: `${dictionary?.datatable?.column?.suggestion}`
      }),
        ...
        cell: ({ row }) => (
          <>
            <Tooltip title='View Details'>
              <IconButton onClick={() => handleOpen(row.original.suggestion)}>
  ...
  return (
    <Card>
      ...
      <Dialog open={open} onClose={handleClose} fullWidth maxWidth='sm'>
        ...
        <DialogContent>
          <p>{selectedSuggestion || 'No details available'}</p>
        </DialogContent>
        <DialogActions>
...
export default SuggestionsOnMenu
