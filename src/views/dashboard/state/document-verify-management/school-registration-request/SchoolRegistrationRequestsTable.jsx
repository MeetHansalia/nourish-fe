...
const SchoolRegistrationRequestsTable = ({ dictionary }) => {
  ...
  const getAllRequests = async () => {
    ...
    try {
      const response = await axiosApiCall.get(API_ROUTER.STATE.GET_SCHOOL_REGISTRATION_REQUESTS, {
        params: {
          page,
          limit: itemsPerPage,
          ...
          <IconButton>
            <Link
              href={getLocalizedUrl(
                `/${panelName}/document-verify/school-registration-request/${row?.original?._id}`,
                locale
              )}
              className='flex'
            >
