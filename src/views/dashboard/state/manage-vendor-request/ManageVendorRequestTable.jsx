...
const ManageVendorRequest = ({ dictionary }) => {
  ...
  const [lastMomentcancelationData, setLastMomentCancelationData] = useState(0)
  ...
  const getAllRequests = async () => {
    ...
    try {
      const response = await axiosApiCall.get(API_ROUTER.STATE.GET_ALL_CANCEL_ORDER, {
        params: {
          page,
          limit: itemsPerPage,
  ...
  const getLastMomentCancelationStatistic = async () => {
    ...
    await axiosApiCall
      ...
      .then(response => {
        setLastMomentCancelationData(response?.data?.response?.lastMomentCancelOrders)
    ...
    actionConfirmWithLoaderAlert(
      {
         ...
         */
        deleteUrl: API_ROUTER.STATE.APPROVE_REJECT_VENDOR_ORDER_REQUEST(vendorData?._id),
