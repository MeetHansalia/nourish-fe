...
const RejectConfirmationDialogBox = ({ open, setOpen, vendorData, getAllRequests, page, itemsPerPage, dictionary }) => {
  ...
  const handleReject = async () => {
    ...
    try {
      const response = await axiosApiCall.patch(API_ROUTER.STATE.APPROVE_REJECT_VENDOR_ORDER_REQUEST(vendorData?._id), {
        vendorId: vendorData?.vendorId?._id,
        cancelorderRequestRejectReason: reason,
  ...
  return (
    <Dialog open={open} onClose={() => setOpen(false)} fullWidth maxWidth='sm'>
      {console.log('selectedRow', vendorData)}
      <DialogTitle>{dictionary?.page?.common?.order_reject_text}</DialogTitle>
      <DialogContent>
        <Typography variant='body1' mb={2}>
          {dictionary?.page?.common?.order_reject_title}
        </Typography>
        <TextField
