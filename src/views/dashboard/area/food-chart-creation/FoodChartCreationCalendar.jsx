...
const FoodChartCreation = props => {
  ...
  // console.log(schoolAdminId, groupId, 'IDs')
  ...
  useEffect(() => {
    if (groupId && schoolAdminId) {
      getChartData()
    }
  }, [groupId, schoolAdminId])
  ...
  const getChartData = () => {
    ...
    axiosApiCall
      .get(API_ROUTER.AREA_EXECUTIVE.GET_FOODCHART_DATA, {
      ...
      .then(response => {
        ...
        const startDate = responseBodyData?.startDate
        const endDate = responseBodyData?.endDate
  ...
  const onCalendarEventSubmitHandler = () => {
    ...
    axiosApiCall
      .post(API_ROUTER.AREA_EXECUTIVE.POST_AREA_FOOD_CHART, apiFormData)
      .then(response => {
        const responseBody = response?.data
        const responseBodyData = responseBody?.response
                ...
                <Grid container spacing={6}>
                  ...
                  <Grid item lg={4}>
                    <AppReactDatepicker
                      ...
                      customInput={<CustomTextField fullWidth />}
                      placeholderText={t('form.label.school_name')}
                      disabled
                    />
                  </Grid>
                  {isChartUpdated && (
