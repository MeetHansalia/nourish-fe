...
const Menu = ({ dictionary, kidId, vendorId }) => {
  ...
  const fetchLatestCartDetails = async kidId => {
    try {
      const response = await axiosApiCall.get(API_ROUTER.AREA_EXECUTIVE.GET_CART_DETAILS(orderId))
  ...
  const CategoryApiCall = async vendorId => {
    await axiosApiCall
      .get(API_ROUTER.AREA_EXECUTIVE.GET_ALL_CATEGORIES, { params: { vendorId: vendorId } })
      .then(response => {
