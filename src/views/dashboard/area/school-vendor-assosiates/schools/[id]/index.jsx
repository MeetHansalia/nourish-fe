 ...
 * Page
 */
const EditDetails = ({ dictionary, params }) => {
  ...
  const [phoneInputCountry, setPhoneInputCountry] = useState()

  const getSchoolVendorById = async () => {
    await axiosApiCall
      .get(API_ROUTER.STATE.GET_VENDOR_SCHOOL_LIST_BY_ID(params.id))
      .then(response => {
  ...
  useEffect(() => {
    if (userData?._id) {
      ...
      setValue('countryCode', userData?.countryCode || PHONE_NUMBER_DEFAULT_COUNTRY_CODE)

      if (!phoneInputCountry) {
        setPhoneInputCountry(userData?.countryCode || PHONE_NUMBER_DEFAULT_COUNTRY_CODE)
      }
  ...
  const onSubmit = async data => {
    ...
    axiosApiCall
      .patch(API_ROUTER.STATE.UPDATE_SCHOOL(params.id), apiFormData)
      .then(response => {
