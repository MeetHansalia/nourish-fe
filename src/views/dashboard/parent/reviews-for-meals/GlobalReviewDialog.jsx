...
const GlobalReviewDialog = () => {
  ...
  const onClickLaterReview = async () => {
    try {
      const res = await axiosApiCall.post(API_ROUTER.PARENT.REVIEW_LATER)
  ...
  const getPendingReviewList = async () => {
    try {
      const res = await axiosApiCall.get(API_ROUTER.PARENT.GET_PENDING_REVIEW_LIST)
  ...
  const handleDialogClose = async e => {
    ...
    if (pathnameWithoutPanel === '/reviews-for-meals') {
      setIsOpenDialog(false)
      dispatch(setIsCompleteOrderApiCall(!isCompleteOrderApiCall))
    } else {
      setIsOpenDialog(false)
      window.location.href = '/parent/reviews-for-meals'
    }
  }
