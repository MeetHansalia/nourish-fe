...
const Menu = ({ dictionary, kidId, vendorId }) => {
  ...
  useEffect(() => {
    axiosApiCall
      .get(API_ROUTER.PARENT.GET_PARENT_ALL_CATEGORIES, {
        params: {
          vendorId,
    ...
    <div className='multi-order-custom'>
      <div className='common-block-dashboard p-0'>
        ...
        <div className='card-category-img'>
          <CardContent className='flex gap-4 p-0 overflow-x-auto'>
            <div
            ...
            {categories.map(category => (
              <div
                key={category._id}
                className={`card-category-img-common cursor-pointer flex items-center gap-2 ${
                  ...
                  <div className='flex items-center gap-4'>
                    ...
                    <div>
                      ...
                      {item.modifiers.length > 0 && (
                        <Typography variant='body2' color='textSecondary'>
                          {dictionary?.form?.placeholder?.modifiers}: {item.modifiers.map(mod => mod?.dishId?.name).join(', ') || 'None'}
                        </Typography>
                      )}
                    </div>
                  </div>
