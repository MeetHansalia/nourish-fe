...
const OrderDataTable = props => {
  ...
  const getAllCompletedOrders = async () => {
    ...
    try {
      const response = await axiosApiCall.get(API_ROUTER.PARENT.TRAKING_ORDERS, {
        params: {
          page,
          limit: itemsPerPage,
  ...
  const getVendorData = async () => {
    ...
    try {
      const response = await axiosApiCall.get(API_ROUTER.PARENT.VENDOR_LIST)
  ...
  const columns = useMemo(
    () => [
      ...
      columnHelper.accessor(row => `${row.vendorId.companyName}`, {
        ...
        <CardContent className='p-0 flex flex-col gap-4'>
          ...
          >
            <Grid item xs={12} md={4}>
              <Typography variant='h4'>{dictionary?.datatable?.order_tracking?.table_title}</Typography>
            </Grid>
