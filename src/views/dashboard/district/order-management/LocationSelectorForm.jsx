...
const LocationSelectorForm = ({
  ...
  const fetchSchoolList = (countryName, stateName, districtName) => {
    ...
    setSchools([])
    axiosApiCall
      .get(`${API_ROUTER.DISTRICT.GET_SCHOOLS}?${queryParams.toString()}`, {
  ...
  useEffect(() => {
    fetchSchoolList(user?.location?.country, user?.location?.state, user?.location?.district)
  }, [])
  ...
  const handleClick = () => {
    ...
    if (!school) {
      console.log('All fields must be selected before proceeding.')
