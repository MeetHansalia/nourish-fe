...
const DocumentsForVerification = props => {
  ...
  const getSubmittedDocuments = async () => {
    setIsLoading(true)
    await axiosApiCall
      .get(API_ROUTER.DISTRICT.GET_SPECIFIC_DOCUMENT(id), {
        // params: { userId: id }
      })
      .then(response => {
    ...
    actionConfirmWithLoaderAlert(
      {
         ...
         */
        deleteUrl: API_ROUTER.DISTRICT.PATCH_APPROVE_REJECT_DOCUMENT,
  ...
  const handleReject = async () => {
    ...
    await axiosApiCall
      .patch(API_ROUTER.DISTRICT.PATCH_APPROVE_REJECT_DOCUMENT, requestBody)
      .then(response => {
